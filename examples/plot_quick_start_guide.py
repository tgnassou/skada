...
#
# .. image:: /_static/images/image_classif3.png
#    :width: 400px
#    :align: center
#
...
# Plotting helpers used throughout this example. They are kept out of the
# rendered page (via the ignore markers) so the focus stays on the DA
# concepts and the SKADA API rather than on matplotlib boilerplate.


def plot_shift_types(shifts, shift_names):
...
def plot_subspace_shift(shift_name):
    ...
    axes[1].set_title(shift_name, fontsize=12)
...
def plot_accuracy_drop(shifts, shift_names):
...
def plot_subspace_accuracy_drop(shift_name):
...
def plot_reweighting_steps(dataset, estimator):
...
def plot_mapping_steps(dataset, estimator):
    fig, axes = plt.subplots(1, 4, figsize=(8, 2.2), sharex=True, sharey=True)
    ...
    X_adapted = estimator[:-1].transform(
...
def plot_subspace_steps(dataset, clf):
    fig, axes = plt.subplots(1, 4, figsize=(8, 2.2), sharex=True, sharey=True)
    X, y, sample_domain = dataset.pack(
        as_sources=["s"], as_targets=["t"], mask_target_labels=True
    )
    ...
    y_source_c = np.where(y[idx] == 0, color_0, color_1)
    subspace_cmap = ListedColormap([color_0, color_1])
    ...
    axes[2].contourf(xx, yy, Z, alpha=0.3, cmap=subspace_cmap)
    ...
    y_target_c = np.where(y[~idx] == 0, color_0, color_1)
    axes[3].contourf(xx, yy, Z, alpha=0.3, cmap=subspace_cmap)
    axes[3].scatter(X[~idx, 0], X[~idx, 1], c=y_target_c, alpha=0.5)
    axes[3].set_title("Prediction on Target", fontsize=12)
...
plot_accuracy_drop(shifts, shift_names)
...
plot_subspace_accuracy_drop(shift_names[-1])
...
plot_reweighting_steps(dataset, estimator)
...
plot_mapping_steps(dataset, estimator)
...
plot_subspace_steps(dataset, clf)
# %%
